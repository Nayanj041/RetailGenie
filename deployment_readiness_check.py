...
def check_backend_app():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "app_module", "/workspaces/RetailGenie/backend/app.py"
        )
        ...
        with app.test_client() as client:
            ...
            if result and "token" in result and "user" in result:
                ...
                if result and result.get("success"):
                    ...
                    print(
                        f"❌ Registration failed: {result.get('error') if result else 'No response'}"
                    )
                    return False
            else:
                print(
                    f"❌ Registration endpoint error: {result.get('error') if result else 'Unknown error'}"
                )
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            if response.status_code in [
                200,
                404,
            ]:  # 404 is OK if product not found but endpoint works
                result = response.get_json()
