...
class AuthTester {
  ...
  async testRegistration() {
    ...
    try {
      const response = await this.makeRequest(`${API_BASE_URL}/api/v1/auth/register`, {
        method: 'POST',
        body: userData
      });
  ...
  async testLogin(email = 'demo@retailgenie.com', password = 'demo123456') {
    ...
    try {
      const response = await this.makeRequest(`${API_BASE_URL}/api/v1/auth/login`, {
        method: 'POST',
        body: loginData
      });
  ...
  async testAuthenticatedRequest() {
    ...
    try {
      const response = await this.makeRequest(`${API_BASE_URL}/api/v1/products`, {
        method: 'GET',
        headers: {
  ...
  printSummary() {
    ...
    const tests = [
      ...
      { name: 'Authenticated Requests', result: this.testResults.authenticated }
    ];
