...
window.authTests = {
  ...
  async testAuthenticatedRequest(token = null) {
    ...
    const authToken =
      token || localStorage.getItem("authTestToken") || window.currentToken;
  ...
  async runFullTest() {
    ...
    if (results.registration) {
      results.login = await this.testLogin(
        window.testUser.email,
        window.testUser.password,
      );
    } else {
    ...
    console.log(
      `Authenticated Requests: ${results.authenticated ? "✅" : "❌"}`,
    );
...
console.log(
  "  authTests.testAuthenticatedRequest() - Test authenticated API call",
);
