...
def add_advanced_endpoints(
    app, firebase, controllers, require_auth, get_json_data, logger
):
    ...
    def search_products():
        ...
        try:
            ...
            if not query:
                return (
                    jsonify({"success": False, "error": "Search query required"}),
                    400,
                )
    ...
    def get_product_categories():
        ...
        try:
            ...
            for category in categories:
                ...
                category_stats.append({"name": category, "product_count": count})
            ...
            if "inventory" in controllers:
                ...
                for product in products:
                    ...
                    status = (
                        "out_of_stock"
                        if stock == 0
                        else "low_stock" if stock < 10 else "in_stock"
                    )
                    if status == "low_stock":
                        low_stock_count += 1
                ...
                return jsonify(
                    {
                        ...
                        "summary": {
                            ...
                            "out_of_stock_count": len(
                                [p for p in products if p.get("stock", 0) == 0]
                            ),
                        },
                    }
                )
    ...
    def update_inventory():
        ...
        try:
            ...
            if "inventory" in controllers:
                ...
                if not updates:
                    return (
                        jsonify({"success": False, "error": "No updates provided"}),
                        400,
                    )
            ...
            return jsonify(
                {
                    ...
                    "summary": {
                        ...
                        "average_order_value": (
                            round(total_revenue / len(orders), 2) if orders else 0
                        ),
                    },
        ...
        try:
            ...
            for item in items:
                if not all(k in item for k in ["product_id", "quantity"]):
                    return (
                        jsonify({"success": False, "error": "Invalid item format"}),
                        400,
                    )
