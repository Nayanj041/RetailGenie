...
def create_app():
    ...
    def get_products():
        try:
            if firebase:
                ...
                return jsonify(
                    {"success": True, "count": len(products), "products": products}
                )
            else:
                # Return mock data
                mock_products = [
        ...
        try:
            ...
            if firebase:
                ...
                doc_id = firebase.create_document(
                    "products", product_data, product_data["id"]
                )
                return (
                    jsonify(
                        {
    ...
    def get_customers():
        try:
            if firebase:
                ...
                return jsonify(
                    {"success": True, "count": len(customers), "customers": customers}
                )
            else:
                mock_customers = [
                    {
    ...
    def init_database():
        try:
            if not firebase:
                return (
                    jsonify({"success": False, "error": "Database not connected"}),
                    500,
                )
