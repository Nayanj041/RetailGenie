...
def add_all_advanced_endpoints(
    app, firebase, controllers, require_auth, get_json_data, logger, generate_jwt_token
):
    ...
    def get_inventory_status():
        ...
        try:
            if "inventory" in controllers:
                ...
                total_value = sum(
                    p.get("price", 0) * p.get("stock", 0) for p in active_products
                )
                ...
                out_of_stock_items = [
                    p for p in active_products if p.get("stock", 0) == 0
                ]
        ...
        try:
            ...
            if "ai" in controllers:
                ...
                category_recommendations = recommendations.get(
                    category, recommendations["general"]
                )
        ...
        try:
            if "ai" in controllers:
                ...
                if active_products:
                    ...
                    avg_price = sum(p.get("price", 0) for p in active_products) / len(
                        active_products
                    )
                    total_value = sum(
                        p.get("price", 0) * p.get("stock", 0) for p in active_products
                    ...
                    insights.append(
                        "No products found. Start by adding some products to your catalog."
                    )
            ...
            if "analytics" in controllers:
                ...
                dashboard_data = {
                    "overview": {
                        ...
                        "average_order_value": (
                            sum(o.get("total", 0) for o in orders) / len(orders)
                            if orders
                            else 0
                        ),
                    },
                    "inventory": {
                        ...
                        "low_stock_count": len(
                            [p for p in active_products if p.get("stock", 0) < 10]
                        ),
                    },
                    "recent_activity": {
                        "recent_orders": len(
        ...
        try:
            if "pricing" in controllers:
                ...
                pricing_data = {
                    ...
                    "cost": product.get(
                        "cost", product.get("price", 0) * 0.7
                    ),  # Estimated cost
                    "margin": product.get("price", 0) * 0.3,  # Estimated margin
                    "margin_percentage": 30.0,
                    "recommended_price": product.get("price", 0)
                    * 1.05,  # 5% increase suggestion
                    "competitor_average": product.get("price", 0)
                    * 0.95,  # Estimated competitor price
