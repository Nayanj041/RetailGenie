...
class InventoryForecastingModel:
    ...
    def __init__(self):
        ...
        self.scaler_path = os.path.join(
            os.path.dirname(__file__), "inventory_scaler.pkl"
        )
    ...
    def prepare_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "sales_quantity" in features.columns:
            # Rolling averages
            features["sales_7day_avg"] = (
                features["sales_quantity"].rolling(window=7, min_periods=1).mean()
            )
            features["sales_30day_avg"] = (
                features["sales_quantity"].rolling(window=30, min_periods=1).mean()
            )

            # Trend indicators
            features["sales_trend"] = features["sales_quantity"].pct_change(periods=7)
            features["sales_volatility"] = (
                features["sales_quantity"].rolling(window=7).std()
            )
        ...
        if "price" in features.columns:
            ...
            features["price_elasticity"] = (
                features.get("sales_quantity", 0) / features["price"]
            )

        # Inventory features
        if "current_stock" in features.columns:
            features["stock_level"] = features["current_stock"]
            features["days_of_inventory"] = features["current_stock"] / (
                features.get("sales_7day_avg", 1) + 1
            )
    ...
    def train(
        self, training_data: pd.DataFrame, target_column: str = "future_demand"
    ) -> Dict:
        """
    ...
    def optimize_inventory(self, products: List[Dict]) -> Dict:
        ...
        for product in products:
            ...
            recommendation = {
                ...
                "estimated_cost": self._estimate_cost(
                    product, recommended_stock - current_stock
                ),
            }
        ...
        return {
            ...
            "total_investment_needed": sum(
                r["estimated_cost"] for r in recommendations if r["estimated_cost"] > 0
            ),
    ...
    def _get_action_recommendation(
        self, current_stock: float, recommended_stock: float
    ) -> str:
    ...
    def _generate_summary(self, recommendations: List[Dict]) -> Dict:
        ...
        urgent_items = len(
            [r for r in recommendations if r["action"] == "urgent_restock"]
        )
        restock_items = len(
            [
                r
                for r in recommendations
                if r["action"] in ["urgent_restock", "restock_soon"]
            ]
        )
        ...
        return {
            ...
            "percentage_needing_action": (
                (restock_items / total_products * 100) if total_products > 0 else 0
            ),
        }
