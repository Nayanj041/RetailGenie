...
def create_app():
    ...
    cors_origins = os.getenv(
        "CORS_ORIGINS",
        "http://localhost:3000,http://localhost:3001,http://127.0.0.1:3000,https://retailgenie-1.onrender.com",
    ).split(",")
            ...
            "authentication": [
                {
                    ...
                    "method": "POST",
                    "description": "Retailer registration (retailer-only platform)",
                },
                {
    ...
    def get_products_advanced():
        ...
        try:
            ...
            if min_price is not None:
                filtered_products = [
                    p for p in filtered_products if p.get("price", 0) >= min_price
                ]
            if max_price is not None:
                filtered_products = [
                    p for p in filtered_products if p.get("price", 0) <= max_price
                ]
        ...
        try:
            if "product" in controllers:
                ...
                if product:
                    ...
                    return (
                        jsonify({"success": False, "error": "Product not found"}),
                        404,
                    )
        ...
        try:
            ...
            if "analytics" in controllers:
                ...
                sales_data = {
                    "period": period,
                    "total_sales": sum(
                        order.get("total", 0) for order in filtered_orders
                    ),
                    "order_count": len(filtered_orders),
                    "avg_order_value": (
                        sum(order.get("total", 0) for order in filtered_orders)
                        / len(filtered_orders)
                        if filtered_orders
                        else 0
                    ),
    ...
    def get_feedback():
        ...
        try:
            if "feedback" in controllers:
                ...
                return (
                    jsonify(
                        {"success": True, "feedback": feedback, "count": len(feedback)}
                    ),
                    200,
                )
    ...
    def generate_smart_ai_response(message, firebase):
        ...
        if any(
            word in message_lower for word in ["product", "inventory", "stock", "item"]
        ):
            product_count = len(products)
            if product_count > 0:
                top_categories = list(
                    set(p.get("category", "Unknown") for p in products)
                )[:3]
        ...
        # Order-related responses
            ...
            word in message_lower for word in ["order", "sale", "purchase", "revenue"]
        ):
        ...
        # Analytics and insights
            ...
            for word in ["analytics", "report", "insight", "performance"]
        ):
        ...
        # Recommendations
            ...
            word in message_lower for word in ["recommend", "suggest", "advice", "help"]
        ):
            if len(products) == 0:
    ...
    def get_analytics():
        ...
        try:
            ...
            conversion_rate = (
                (total_orders / max(total_customers, 1)) * 100
            ...
            try:
                ...
                try:
                    ...
                    if feedback_texts:
                        ...
                        analysis_data = {
                            "analysis": {
                                "overall_sentiment": batch_results["statistics"][
                                    "overall_sentiment"
                                ],
                                "sentiment_distribution": {
                                    "positive": batch_results["statistics"][
                                        "positive_count"
                                    ],
                                    "neutral": batch_results["statistics"][
                                        "neutral_count"
                                    ],
                                    "negative": batch_results["statistics"][
                                        "negative_count"
                                    ],
                                },
                                "trending_topics": [
                                    {
                                        ...
                                        "sentiment": "positive",
                                        "mentions": batch_results["statistics"][
                                            "positive_count"
                                        ],
                                    },
                                    {
                                ...
                                "confidence": batch_results["statistics"][
                                    "average_confidence"
                                ],
                            },
                            "total_feedback": batch_results["statistics"][
                                "total_feedback"
                            ],
                            "generated_at": datetime.now(timezone.utc).isoformat(),
                        }
    ...
    def get_inventory_forecast():
        ...
        try:
            ...
            try:
                from inventory_forecasting.forecast_model import (
                ...
                try:
                    inventory_docs = (
                        firebase.get_documents("inventory")
                        or firebase.get_documents("products")
                        or []
                    )
    ...
    def optimize_pricing():
        ...
        try:
            ...
            if not product_id:
                return (
                    jsonify({"success": False, "error": "Product ID is required"}),
                    400,
                )
            ...
            try:
                ...
                try:
                    ...
                    cost = product_doc.get(
                        "cost", current_price * 0.6
                    )  # Assume 40% margin if cost not provided

                    # Simple pricing optimization logic
                    market_factor = 1.0  # Could be influenced by competitor prices
                    demand_factor = (
                        1.1 if product_doc.get("quantity", 0) < 10 else 0.95
                    )  # Higher price if low stock
